...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    _cnt = 0
    for samples, events, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, events, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        samples = samples.to(device)
...
def test(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        print(f'Done. ({(end_time - start_time) * 1000:.3f}ms)')
        # loss_dict = criterion(outputs, targets)
        # weight_dict = criterion.weight_dict
